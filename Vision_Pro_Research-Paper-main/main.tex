...
This paper presents a smart locking system that integrates face recognition with liveness detection to provide secure authentication and mitigate spoofing attacks. The proposed architecture employs a stereo vision framework to estimate facial depth via epipolar geometry, effectively distinguishing between actual faces and 2D presentation attacks. Upon successful liveness verification, the face recognition module, authenticates the user. Once the identity is confirmed, the locking mechanism is disengaged. An interactive web interface facilitates remote user management and real-time monitoring. The experimental evaluations demonstrate that the proposed solution achieves high accuracy and an ultra-low latency, validating its feasibility for real-time, robust, and secure access control.
\end{abstract}
...
Vongchumyen et al. \cite{vongchumyen2017} developed a Web-based solution, while Aswini et al. \cite{aswini2021} introduced an RFID-OTP system. While these solve the lost-key problem, they fail to offer true convenience. Whether using a phone app or scanning a tag, the user is required to manually interact with a device, negating the hands-free benefit of home automation.

\subsection{Monocular Biometric Systems}
To improve convenience, facial recognition was adopted, but single-camera systems face a trade-off between security and speed. Cloud-based solutions by Maheshwari and Nalini \cite{maheshwari2017} offer accuracy but suffer from latency and internet dependency. Alternatively, local offline systems like Elechi et al. \cite{elechi2022} often lack the robustness to distinguish real faces from photos. To counter this, Dasare \cite{dasare2021} introduced active liveness (hand gestures), which unfortunately adds user friction. A robust system must be both passive (no gestures) and local (offline).
...
During the inference phase, the system verifies identity by comparing the embedding of the live user ($\mathbf{p}$) against the stored enrolled embedding ($\mathbf{q}$). Since the vectors are normalized, the semantic similarity is measured using the Euclidean Distance Eq.\ref{eq:euclidean}:

\begin{equation}
d(\mathbf{p}, \mathbf{q}) = ||\mathbf{p} - \mathbf{q}||_2 = \sqrt{\sum_{i=1}^{128} (p_i - q_i)^2}.
\label{eq:euclidean}
\end{equation}

A match is confirmed if the distance falls below a specific threshold $\tau$, indicating high confidence that both images belong to the same individual.
...
\begin{figure*}[t!]
    ...
    \begin{subfigure}{0.45\textwidth}
        ...
        \caption{\textbf{Integrated System Architecture: }System components with data flow.}
        \label{fig:sys2}
    \end{subfigure}
...
Facial landmarks are detected using the YuNet model \cite{Wu2023YuNet}, which identifies both bounding boxes and precise coordinates for key facial features. These landmarks are detected in the rectified image space and mapped to the corresponding coordinates in the disparity map. This mapping enables direct association between facial landmarks and their estimated depth values.
...
where $Z_{\text{left\_eye}}$, $Z_{\text{right\_eye}}$, and $Z_{\text{nose}}$ denote the estimated depths at the corresponding facial landmark. The liveness condition is satisfied if the resulting geometric protrusion exceeds a predefined threshold. The threshold $[\Delta Z_{min}, \Delta Z_{max}]$ defined based on the empirical separation between planar spoofs ($\approx 0\text{m}$) and actaul facial depth profiles ($>0.020\text{m}$), as in Appendix. This range effectively rejects both 2D presentation attacks and depth estimation outliers while accommodating natural facial variations. To ensure robustness against noise and transient estimation errors, liveness verification is performed over 3 consecutive frames.


\textbf{Face Recognition.}
The face recognition subsystem follows a sequential three-stage pipeline: detection, geometric alignment, and feature extraction. The input image $I$ is first processed by YuNet \cite{Wu2023YuNet}, an efficiency-optimized convolutional neural network. For every detected face, the model outputs a bounding box $B = (x, y, w, h)$ and a set of five landmarks $L_{src} \in \mathbb{R}^{5 \times 2}$:

\begin{equation}
\begin{split}
L_{src} = \{ (x_i, y_i) \mid i \in \{ &\text{left\_eye}, \text{right\_eye}, \text{nose}, \\
&\text{left\_mouth}, \text{right\_mouth} \} \}.
\end{split}
\label{eq:landmarks}
\end{equation}

To standardize the input for recognition, we apply a Affine Transformation \cite{Deng2019ArcFace} to map the detected facial landmarks to a canonical frontal template. This transformation accounts for rotation ($\theta$), isotropic scaling ($s$), and translation ($t_x, t_y$) via the matrix $M$ Eq.\ref{eq:affine_matrix}:
...
Finally, the aligned image $I_{aligned}$ (fixed to $112 \times 112$ pixels) is generated by warping the original pixels using the inverse transformation:

\begin{equation}
I_{aligned}(x', y') = I( M^{-1} [x', y', 1]^T ).
\end{equation}

The aligned image is passed to SFace \cite{Zhong2021SFace}, which maps the high-dimensional pixel data into a compact 128-dimensional embedding vector $\mathbf{v} \in \mathbb{R}^{128}$. These embeddings are normalized to lie on a hypersphere, ensuring that the Euclidean distance between vectors corresponds directly to semantic similarity.
...
\begin{equation}
S = \frac{u \cdot v}{\lVert u \rVert \, \lVert v \rVert}.
\end{equation}

where $u$ represents the extracted facial feature vector and $v$ denotes a stored enrollment embedding. Access is granted only if the similarity score exceeds a threshold, thereby identifying the user with high confidence.

\textbf{Hardware Actuation.}
The system effectively governs transitions between liveness verification, recognition, and access states. Upon successful authentication, a command signal is transmitted via serial communication to an Arduino, which actuates a solenoid lock. A cooldown interval of 5 seconds is enforced to prevent repeated triggering.
...
\subsection{Dataset}
We collect the initial dataset using a laptop web camera. The dataset includes three participants with 200 high-quality images, each extracted from 60-second videos where participants perform various head movements and facial expressions. To ensure data security, the system temporarily stores images locally on the Raspberry Pi. For evaluation, we use a test dataset consisting of 150 images. The model converts each image into a 128-dimensional vector, generates embeddings for all training images, and averages them to create a mean embedding for each identity. The system saves these embeddings locally on the Raspberry Pi 5 to support real-time recognition and evaluation.
...
\begin{table}[t!]
\centering
\caption{Model Evaluation (Standard Metrics)}
\resizebox{\linewidth}{!}{%
...
Among the deep learning approaches, Dlib (OpenCV) achieved improved recognition performance (Recall 0.83), but it was accompanied by a significant security concern: a high False Positive rate (FP 0.55). This indicates a lack of discriminative ability to differentiate between users and impostors. Although InsightFace demonstrated high Accuracy (0.94) and perfect Precision (1.00), it was not selected due to hardware constraints. Its computationally intensive architecture exceeded the capabilities of the Raspberry Pi 5 and rendered it unsuitable for edge deployment.

Ultimately, YuNet + SFace provided the optimal solution. It achieved perfect metrics (1.00 Accuracy, 1.00 Precision, 0.00 FP), demonstrating the ideal balance of computational efficiency and high-precision security required for the final system.
...
The YuNet + SFace configuration emerged as the optimal architecture. It achieved perfect accuracy (1.0) and security (0.0 FP) and also maintained an ultra-low latency of 15 ms. This confirms that YuNet + SFace successfully bridges the gap between high-performance security and real-time efficiency, validating its selection as the core engine for our embedded system.
