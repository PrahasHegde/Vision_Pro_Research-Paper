...
To reconstruct 3D information, we model the camera using the Pinhole Model. First, we estimate the \textbf{Intrinsic Matrix} ($K$) using the flexible calibration technique established by \textbf{Zhang (1998)}. This accounts for focal length ($f_x, f_y$) and optical center ($c_x, c_y$) from low-cost sensors:
...
For stereo consistency, we utilize the \textbf{Epipolar Constraint} and triangulation framework described by \textbf{Hartley and Sturm (1997)}. Given a point projected onto the left ($\mathbf{p}_L$) and right ($\mathbf{p}_R$) image planes, their relationship is governed by the Fundamental Matrix ($\mathbf{F}$):
...
To compute depth, we first calculate pixel disparity ($d$) using the \textbf{Semi-Global Block Matching (SGBM)} algorithm. Unlike simple pixel matching, SGBM enforces a global smoothness constraint to reduce noise on textureless surfaces like skin. The depth $Z$ is inversely proportional to disparity:
...
Identity verification utilizes a neural network trained with \textbf{Triplet Loss} (\textbf{Schroff et al., 2015}) to map facial features into a 128-dimensional embedding space. Verification is performed by calculating the \textbf{Euclidean Distance} between the live embedding ($\mathbf{p}$) and stored embedding ($\mathbf{q}$):
...
\begin{figure*}[t!]
    ...
    \begin{subfigure}{0.5\textwidth}
        \centering
        \begin{subfigure}{0.5\textwidth}
            \includegraphics[width=\textwidth]{img/mes_lab.png}
            \caption{\textbf{3D Reconstruction:}}
            \label{fig:sys1a}
        \end{subfigure}%
...
The proposed system comprises a sequential processing pipeline designed for real-time secure access control. The workflow begins with stereo vision for depth perception and anti-spoofing, followed by deep learning-based face recognition and a control interface for hardware actuation. The system captures synchronized video streams from two network cameras, applies epipolar rectification and depth estimation to verify facial liveness, and subsequently performs identity recognition using learned facial embeddings. After successful authentication, an Arduino-controlled solenoid lock is actuated to grant physical access.

\subsection{System Design and Implementation }
\begin{itemize}
    \item \textbf{Epipolar Geometry}: The stereo vision module follows the calibrated stereo camera model described in Section~III. Intrinsic and extrinsic parameters are obtained offline using a standard chessboard calibration procedure. Using these parameters, epipolar rectification is applied to align corresponding scanlines, enforcing the epipolar constraint defined in Eq.~\ref{eq:epipolar_dots}. Disparity is then computed along horizontal epipolar lines using the Semi-Global Block Matching (SGBM) algorithm, producing a dense disparity map suitable for real-time processing.
...
After depth estimation, facial landmarks are detected using the YuNet model, which identifies both bounding boxes and precise coordinates for key features such as the eyes and nose tip. These landmarks are detected in the rectified image space and mapped to the corresponding coordinates in the disparity map. This mapping enables direct association between facial landmarks and their estimated depth values, forming the basis for subsequent liveness analysis.

\item \textbf{Liveness Detection Using Landmark-Based Depth Analysis}:
A human face is a 3d object where the nose is closer to the camera than eyes. Conversely, a 2D spoof (photo) is planar, meaning the depth of the nose and eyes is approximately equal. Depth values are extracted at the landmark positions corresponding to the left eye, right eye, and nose tip.
...
where $Z_{\text{left\_eye}}$, $Z_{\text{right\_eye}}$, and $Z_{\text{nose}}$ denote the estimated depths at the corresponding facial landmark locations. The liveness condition is considered satisfied if the resulting geometric protrusion exceeds a predefined threshold. To ensure robustness against noise and transient estimation errors, liveness verification is performed over multiple consecutive frames.


\item \textbf{Face Recognition and Identity Verification}:
After successful liveness verification, the system employs a deep learning pipeline consisting of two models: YuNet for face detection and SFace for face recognition. To ensure robustness against pose variations, Affine transformation is applied using the facial landmarks provided by YuNet. This transformation aligns the detected face by correcting rotational offsets before it is passed to SFace, a lightweight neural network model. SFace converts the aligned facial image into a compact 128-dimensional embedding vector that represents the user’s unique facial characteristics.
...
\item \textbf{Access Control Logic and Hardware Actuation}:
The system operates as a finite-state machine that governs transitions between liveness verification, recognition, and access decision states. Upon successful authentication, a command signal is transmitted via serial communication to an Arduino, which actuates a solenoid-based locking mechanism. A cooldown interval is enforced to prevent repeated or unintended triggering.
\end{itemize}
...
\subsection{Baseline}
As stated in the above sections, we have employed the YuNet \cite{Wu2023YuNet} and SFace model \cite{Zhong2021SFace} for handling the face detection and recognition task successfully and giving accurate results.
In this section, we are conducting a comparative analysis of our model alongside several others that we have explored. The evaluation metrics focused on True Acceptance Rate (TAR), False Acceptance Rate (FAR), False Rejection Rate (FRR), and overall System Accuracy. Our examination includes traditional approaches such as the Haar Cascade Classifier \cite{voilajonas} and the Histogram of Oriented Gradients in conjunction with support vector machine (SVM) techniques \cite{dalaltriggs}. Additionally, we have evaluated deep learning-based models, including the Dlib facial recognition library in Python \cite{King2009Dlib} and the cutting-edge Insightface (Arcface) framework \cite{Deng2019ArcFace}.
...
\begin{table}[htbp]
\centering
\caption{Models used and Evaluation}
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Models/ Metrics} & \textbf{TAR} & \textbf{FAR} & \textbf{FRR} &\textbf{System Accuracy}\\
\hline
Haar Cascade & 0.12 & 0.70 & 0.87 & 0.08\\
HOG + SVM & 0.0 & 0.13 & 0.86 & 0.52 \\
Dlib (OpenCV) & 0.83 & 0.55 & 0.0 & 0.60 \\
Insightface\textsuperscript{*} & 0.90 & 0.0 & 0.10 & 0.94 \\
YuNet + SFace & 1.0 & 0.0 & 0.0 & 1.0 \\
\hline
\end{tabular}
\label{tab:example_5x5}
\end{table}

\vspace{0.3em}

\begin{flushleft}
\footnotesize * indicates that the training/testing was conducted on a PC instead of the Raspberry Pi 5, due to the computational intensity and hence model was not considered for the final project.
\end{flushleft}
...
\begin{figure}[htbp]
    ...
    \includegraphics[width=0.5\textwidth]{img/model_cf.png}
    \caption{Simple block diagram of the system}
    \label{fig:cf}
\end{figure}
...
\begin{tabular}{|l|c|c|c|>{\RaggedRight}p{1.5cm}|}
\hline
\textbf{Configuration/Metrics} & \textbf{Accuracy} & \textbf{FAR} & \textbf{Latency} & \textbf{Comments} \\
\hline
...
\hline
Retinaface + Arcface\textsuperscript{*} & 1.0 & 0.0 & 180 ms & Too heavy, accurate but impractical for real-time edge use. \\
\hline
YuNet + SFace & 1.0 & 0.0 & 15 ms & \textbf{Optimal}, perfect balance of speed, size, and security. \\
\hline
\end{tabular}
\label{tab:ablation}
\end{table}
...
To further validate the specific architectural components of our proposed system, we conducted an ablation study to isolate the impact of the detector (YuNet) and the recognizer (SFace) independently. We tested four configurations: YuNet + LBPH, Haar Cascade + SFace, Retinaface + Arcface, and the proposed YuNet + SFace.\\
We first assessed the effect of employing a traditional texture-based recognizer by evaluating the YuNet + LBPH configuration. Despite the integration of a robust detector, system accuracy decreased markedly to 0.45, accompanied by a high False Acceptance Rate (FAR) of 0.40. These results indicate that traditional Local Binary Pattern Histograms (LBPH) lack the necessary feature extraction depth for secure identification, underscoring the need for a deep learning-based recognizer such as SFace.\\

In contrast, we explored the impact of utilizing a less robust detector by pairing Haar Cascade with SFace. Although this setup achieved low latency (14 ms), the accuracy was constrained to 0.68. The system performance was hindered by the Haar Cascade detector's frequent failure to detect faces, resulting in a significant bottleneck. These findings confirm that traditional detectors are inadequate for providing reliably consistent input to the recognition pipeline.\\
...
\textbf{Depth difference between eyes and nose $\Delta z$ (m)} \\
\hline
...
We also extend our heartfelt thanks to Prof. Dr. Andreas Lehrmann for his valuable guidance, continuous support, and insightful feedback throughout the development of this work. His expertise and encouragement played a crucial role in shaping both the conceptual design and the successful implementation.
